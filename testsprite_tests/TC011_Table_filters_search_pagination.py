...
async def run_test():
    ...
    try:
        ...
        # Click on 'Filtros' button to open filter options and apply species filter.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/main/div/div/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Add new animal records to enable testing of filtering, search, and pagination.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/header/div/div/nav/button[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Fill in the new record form with data for a dog record and save it.
        frame = context.pages[-1]
        ...
        # Fill in the required Endereço Completo field to proceed with the record creation.
        frame = context.pages[-1]
        ...
        # Click 'Salvar Registro' button to save the new dog record.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/main/div/div/form/div[4]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Fill in 'Telefone' and 'Nome Completo do Tutor' fields and save the record.
        frame = context.pages[-1]
